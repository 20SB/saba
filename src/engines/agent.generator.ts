...
export class AgentGenerator {
  ...
  private async generateBrainFile(plan: DetailedPlan): Promise<GeneratedFile> {
    ...
    // Generate tool imports with sanitized names
    const toolImports = plan.tools_integration_plan
      .map((tool) => {
        const sanitizedName = this.sanitizeToolName(tool.tool_name);
        const className = this.toPascalCase(sanitizedName);
        return `import { ${className}Tool } from "./tools/${sanitizedName}";`;
      })
      .join("\n");

    // Generate tool declarations with sanitized names
    const toolDeclarations = plan.tools_integration_plan
      .map((tool) => {
        const sanitizedName = this.sanitizeToolName(tool.tool_name);
        const className = this.toPascalCase(sanitizedName);
        const propertyName = sanitizedName.replace(/-/g, "");
        return `private ${propertyName}Tool: ${className}Tool;`;
      })
      .join("\n  ");

    // Generate tool initializations with sanitized names
    const toolInitializations = plan.tools_integration_plan
      .map((tool) => {
        const sanitizedName = this.sanitizeToolName(tool.tool_name);
        const className = this.toPascalCase(sanitizedName);
        const propertyName = sanitizedName.replace(/-/g, "");
        return `this.${propertyName}Tool = new ${className}Tool(this.config, this.logger);`;
      })
      .join("\n    ");
...
`;
  ...
  private async generateToolFiles(plan: DetailedPlan): Promise<GeneratedFile[]> {
    ...
    for (const tool of plan.tools_integration_plan) {
      ...
      files.push({
        path: `src/tools/${sanitizedName}.ts`,
        content,
        type: "TS",
      });
    }
...
`;
  ...
  private generateAgentLogic(plan: DetailedPlan): string {
    const toolExamples = plan.tools_integration_plan
      .map((tool) => {
        const sanitizedName = this.sanitizeToolName(tool.tool_name);
        const propertyName = sanitizedName.replace(/-/g, "");
        return `// await this.${propertyName}Tool.execute({});`;
      })
      .join("\n        ");
